...
/**
 * The agent marks under the setup line. Paths are the official brand marks as
 * shipped by simple-icons v16.29.0 (CC0-1.0, simpleicons.org), inlined so they
 * take the palette's ink and fetch nothing. ChatGPT and Codex share OpenAI's
 * mark: Codex ships no separate one.
 */
const AGENT_MARKS = [
  { name: "Claude", path:
...
export function AgentHandoff({ origin }: { origin: string }) {
  return (
    <section className="mx-auto flex w-full max-w-xl min-w-0 flex-col items-center gap-6 py-12 text-center">
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-muted-foreground">
        <span className="ov-micro ov-micro-sm">Works with</span>
        <ul className="flex items-center gap-3">
          {AGENT_MARKS.map(({ name, path }) => (
            // Title on the item, so the mark names itself on hover as well as
            // to a screen reader.
            <li key={name} title={name} className="transition-colors hover:text-foreground">
              <svg
                viewBox="0 0 24 24"
                width="20"
                height="20"
        ...
        <span className="text-[13px] leading-[1.6]">
          and any agent that reads the Agent Skills format
        </span>
      </div>
    </section>
  );
