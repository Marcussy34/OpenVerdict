...
24. **Stake-weighted draw (§14, §19.2, §24; 2026-09-05):** a staked seat's selection weight is now set from its bond at registration: `weight = min(10_000 * amount / MIN_STAKE_MIST, 100_000)`, so ten times the minimum stake is drawn ten times as often and nothing above that buys more (`agent_registry::stake_selection_weight`). Operator seats registered with a token bond keep the base weight 10,000. The app accepts any amount from 0.1 SUI up to a 1000 SUI ceiling at the prepare step and records the real amount at confirm. Addendum item 19's sentence "Selection weight is equal for every eligible agent in this release" is superseded by this item; reputation-weighted selection remains roadmap.

## 2. Executive summary

OpenVerdict is a decentralized verification protocol for claims that require evidence and judgment rather than a deterministic data feed. A claim receives a proposed outcome and becomes final if nobody challenges it during a defined window. A successful challenge selects a diversity-constrained committee of AI oracle agents (selection weighted by each seat's stake, capped at ten times the minimum; reputation-weighted selection is roadmap), each controlled by a distinct operational signing key and carrying a staked bond. Every agent reviews the same frozen evidence bundle, reasons through GonkaRouter, commits a hidden vote on Sui, and later reveals its answer and argument. If the first round lacks sufficient agreement, agents inspect one another's published evidence and reasoning, add new admissible evidence, and vote again. The protocol can finalize, expand the committee, or remain unresolved instead of manufacturing certainty.
