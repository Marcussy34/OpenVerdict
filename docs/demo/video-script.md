...
| 0:25–0:50 | `/claims/[id]/observe` | Five isolated lanes running live through GonkaRouter. "No agent sees another's work. Each researches the claim through the engine, opens pages on both sides and returns a strict-schema verdict citing what it read; every search, page and model reply is recorded, and every run has a public Gonka Request ID." |
| 0:50–1:10 | Reveal moment | "Votes were sealed on-chain before any reveal — blake2b commitments. Now they open: outcome, confidence, evidence-linked reasoning, model id, Request ID. You can recompute every commitment yourself at /verify, re-run any juror against the same model, and open a sealed bundle through Seal once its deadline has passed." |
