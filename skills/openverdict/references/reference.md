...
- Weather: the engine probes the three model families through GonkaRouter with a research-shaped request, plus the web search provider's credit balance (row `research:firecrawl`, shown as "Web search"), at most once every two minutes; a probe older than five minutes is stale. Weather is "clear" only when the probe is fresh, every model family that still holds an active seat is ok, there are at least as many active families as the registry's draw rule requires, and the web search row is ok. `GET /api/weather` is public and carries both numbers, `requiredFamilies` and `activeFamilies`.
...
- The exception is degraded mode (below): the operator can lower the requirement to two families on chain, and then two healthy active families clear the gate.
...
- Stale and clear. The report is stale when there is no probe or the newest one is older than five minutes (`WEATHER_STALE_MS` 300000). It is clear only when it is not stale, every model family holding an active seat is ok, `activeFamilies.length >= requiredFamilies`, and the web search row is ok. `GET /api/weather` returns `{probedAtMs, stale, clear, families[], requiredFamilies, activeFamilies[]}` with `Cache-Control: no-store`. `requiredFamilies` is read from the registry on chain once per probe (the default 3 when the field was never set, and when a read fails); `activeFamilies` is every family with at least one active eligible seat.
...
The weather strip (the fact-check page, the voided panel on a claim page) shows one chip per row: "healthy" when ok under 30 s of latency, "slow" when ok and slower, "down" when not ok, "no recent probe" when stale, with "probed N s ago" and the legend "A jury needs every active model family (N today) and web search."
...
- Weather gate: a submission needs every model family that still holds an active seat, at least as many of them as the draw requires (three normally, two in degraded mode), and web search up at that moment, and is refused with 503 when they are not.
