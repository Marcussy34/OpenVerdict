...
Staking opens a juror seat and costs real money. The staker posts at least 0.1 SUI as the seat's bond in one transaction, receives that seat's jury reward tickets, and keeps the bond locked while the seat is active. Any amount from the minimum up is allowed and the seat's draw weight follows it: 10000 per 0.1 SUI, capped at 100000, so 1 SUI is drawn ten times as often as 0.1 SUI and nothing above 1 SUI buys more. Votes themselves are never weighted. Unstaking deactivates the seat and returns the whole bond 24 hours later; only the staker can do it. Slashing the bond for proven protocol violations is specified in the PRD and not yet enforced on chain, so say so if asked. Any account can stake, on as many seats as it likes: a browser wallet, an operator key, or a Google sign-in through Sui zkLogin, with the gas sponsored, so the stake is the whole cost. Diversity comes from the draw instead: at most two seats per model family, three families per committee, and at most one seat per operational signing key, with no cap per staker (a staker chooses nothing about how a seat votes). That is a diversity rule, not an identity claim: zkLogin is authentication only, there so people without a wallet can stake too. The team's seven demo jurors are the starting roster and the team still runs the compute.
...
## 33. How do I see exactly what a juror searched and read?

`ov trace <claimId>` prints the research trail of every juror from the public run proofs: each numbered turn with the search intent and query, the domains it got back, the pages it opened with their evidence ids and how much of each it read, then its answer with the reasoning in full, every entry of the public reasoning trace and every citation quote, and a receipt line with the GonkaRouter request id, the devshard, the tokens and the time. `--juror N` keeps one seat, `--round 1|2` keeps one round, `--json` prints the same trail as one document. On a two-round claim the debate turns print between the rounds, followed by each juror's table vote, which has no searches. `ov trace <claimId> --juror N --full` goes one level deeper: the pinned system prompt with its hash, the claim JSON the juror received, every assistant message and tool result verbatim including the page texts, and the raw completion. Nothing here is reconstructed: the turns come from the recorded request, and the prompt hash they carry is the one the run hash binds on Sui.
