...
8. Discussion (only without a quorum). It opens on the fifth reveal (the Move rule also allows the reveal deadline, but under the all-or-nothing policy a missing reveal voids the attempt instead); the discussion deadline at +1560 s is only the bound. Revealed jurors argue in seat order, up to three exchanges, stopping early when nobody moves. The transcript is frozen as phase-two evidence on Walrus and round two opens the moment that bundle is linked.
...
- A verification is one attempt. Any juror error at a binding step voids the whole attempt: a failed run (`INVALID_SCHEMA`, `CITATION_INVALID`, `TIMEOUT`, `PROVIDER_ERROR`), a seat that missed the commit deadline (`MISSING_COMMIT`), or a seat that missed the reveal deadline (`MISSING_REVEAL`). Nothing partial is ever finalized; no vote is ever invented for a failed seat, and the failure record (status, message, research trail, attempts) stays public on Walrus and on the claim page.
...
- Relaunch: the engine relaunches automatically once all three model families answer a health probe (and web search has credits), up to three attempts in total. Relaunches share the queue's ten-minute launch spacing. The relaunched claim is a new claim object linked both ways (`attemptChain.relaunchedAs`, `previousAttempts`). The claim page shows "Attempt N of 3" and a voided banner naming the seat, model and reason.
...
- Discussion, only on a deadlock: the revealed jurors bring their round-one evidence and vote to the table and argue in seat order, streamed live; every turn is its own GonkaRouter run with a current stance and confidence. Up to three exchanges; the debate stops early when nobody moves (`debate_converged`).
...
- Submit on clear or unknown weather: the claim launches at once. Submit on bad weather: the API answers 202 with a queue id, and the queue page shows "Waiting for clear weather" with the live family health.
- Launch: one queued submission per ten-minute window once the weather clears; relaunches of voided attempts share the same spacing, so engine-launched juries never research at the same time (three juries side by side drew a 429 storm from the shared gateway on 2026-09-03).
...
- Decide when claims launch (the queue and the weather gate), pause, deploy, or fail to run at all. Every attempt, void and queue state is public, so a stalled claim is visible.
